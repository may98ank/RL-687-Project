...
on a custom \textbf{cache replacement} environment:

- \textbf{REINFORCE with Baseline} (episodic Monte Carlo policy gradient)
- \textbf{One-Step Actor–Critic} (episodic TD(0)-based policy gradient)
...
2. If \( p_t \) is already in the cache: \textbf{cache hit}.
3. If \( p_t \) is not in the cache:
   - If cache is not full: insert \( p_t \).
   - If cache is full: the agent chooses \emph{which page to evict}.
...
- \( \mathbf{c}_t \in \{0,1\}^N \) encodes cache contents:
  \[
  c_t[i] = \mathbb{1}\{\text{page } i \text{ is in cache at time } t\},
  \]

- \( \mathbf{p}_t \in \{0,1\}^N \) is one-hot for the current request:
  \[
  p_t[i] = \mathbb{1}\{\text{page } i \text{ is requested at time } t\}.
  \]
...
1. \textbf{REINFORCE with Baseline}
2. \textbf{One-Step Actor–Critic}
