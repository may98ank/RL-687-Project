...
def load_policy_and_value_networks():
    """Load trained Actor-Critic policy and value networks for CartPole."""
    policy_net = PolicyNetwork(state_dim=4, action_dim=2, hidden_dim=128)
    value_net = ValueNetwork(state_dim=4, hidden_dim=128)
    
    checkpoint_dir = "checkpoints/cartpole_act_critic"
    policy_path = os.path.join(checkpoint_dir, "policy_net_cartpole_act_critic.pth")
    value_path = os.path.join(checkpoint_dir, "value_net_cartpole_act_critic.pth")
...
def test_policy_and_value_networks():
    """Test loaded Actor-Critic policy and value networks on CartPoleEnv."""
    ...
    eval_results = eval_cartpole_env(env, policy_net, device, num_episodes=1000)
    
    # Print summary statistics
    print(f"\nActor-Critic Evaluation Results for CartPole (over {len(eval_results['episode_rewards'])} episodes):")
    ...
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.title('Episode Rewards (Actor-Critic - CartPole)')
    plt.legend()
    ...
    plt.xlabel('Episode')
    plt.ylabel('Steps')
    plt.title('Episode Steps (Actor-Critic - CartPole)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.title('Reward Distribution (Actor-Critic - CartPole)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.xlabel('Steps')
    plt.ylabel('Frequency')
    plt.title('Steps Distribution (Actor-Critic - CartPole)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    # Save plot
    plot_path = "eval_results_actor_critic_cartpole.png"
